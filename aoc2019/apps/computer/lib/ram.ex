defmodule RAM do
  ...
  def arguments_to_output_pos({[arg], computer}, mode) do
    case mode do
      0 -> {arg, computer}
      2 -> {Map.get(computer, :relative_base) + arg, computer}
    end
    |> (fn u ->
      # IO.inspect(Map.get(computer, :relative_base), label: "base")
  ...
  def arguments_to_values({args, computer}, modes) do
    # IO.inspect(args, label: "input args", charlists: :as_lists)
    # IO.inspect(modes, label: "modes", charlists: :as_lists)
    diff = Enum.count(args) - Enum.count(modes)
    missing_modes = if diff >= 0, do: diff, else: 0
    # IO.inspect(missing_modes, label: "missing modes", charlists: :as_lists)
    values = List.duplicate(0, missing_modes)
      |> Enum.concat(modes)
      |> Enum.reverse()
      |> Enum.zip(args)
  ...
  def extend_memory(memory), do: Enum.concat(memory, List.duplicate(0, 1000000))
